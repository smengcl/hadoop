    ...
    public int doWorkInternal(GetConf tool, String []args) throws IOException {
      ...
      if (!cnnlist.isEmpty()) {
        for (ConfiguredNNAddress cnn : cnnlist) {
          ...
          // formatHostPort brackets a bare IPv6 literal so scripts that parse
          // this output can split host and port unambiguously.
          tool.printOut(NetUtils.formatHostPort(rpc.getHostName(), rpc.getPort()));
        }
        return 0;
      }
